...
```{r ans21}
commit8Date <- commitCheck[[8]][[3]][[2]][[3]]
commit8Date <- as.character(commit8Date)
commit8Date
```
...
```{r ans22}
commit8Date1 <- ymd_hms(commit8Date)
commit8TWDate <- with_tz(commit8Date1,tz="Asia/Taipei")
commit8TWDate
```
...
allCommitDatesList <-as.character(allCommitDatesList)
allCommitDatesList
```
...
```{r ans25}
allCommitDatesList1 <- ymd_hms(allCommitDatesList)
allCommitDatesTW <- with_tz(allCommitDatesList1,tz="Asia/Taipei")
allCommitDatesTW
```
...
```{r ans26}
hourallCommitDatesTW <- hour(allCommitDatesTW)
minuteallCommitDatesTW <- minute(allCommitDatesTW)
logicWithinTimeFrame <- 60*hourallCommitDatesTW+minuteallCommitDatesTW>=270 &60*hourallCommitDatesTW+minuteallCommitDatesTW<=390
...
```{r ans32}
logicFromEcon <- transcriptData[c(3)]=="經濟學系"
logicFromEcon
```
...
```{r ans34}
transcriptPartialDataEcon <- list(transcriptPartialData)
transcriptPartialDataEcon0
```
